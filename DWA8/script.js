...
function preview(props, previewSlice){
    ...
    for(const {props} of previewSlice){
        books.createPreview(props) 
        fragment.appendChild(books.createPreview(props))
    } 
    return books
}

const firstPreview = preview({author, id, image, title }, matches.slice(0, BOOKS_PER_PAGE) ) 
firstPreview.createPreview() 